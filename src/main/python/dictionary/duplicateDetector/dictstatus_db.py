...
class dictstatus_db:
    ...
    def __init__(self):
        ...
        self.hostname = 'lslhdpcd1' # 'localhost'
        self.db_name = ''
        self.JobID_name = ''
        self.table_name = ''
        self.db_JobID = -1
   ...
   # ##################################
    ...
    def set_changeid_fast(self,_cursor,_itemid,_poiid,_filename='',_update_date='',
                          _quiet=False):
        ...
        job_id = self.test_exists(_cursor, sql)
    ...
    def set_changeid(self,_itemid,_poiid,_filename='',_update_date='',
                     _quiet=False):
        # Connect to DB
        ...
        job_id = self.set_changeid_fast(cursor,_itemid,_poiid,_filename,_update_date, _quiet)
    ...
    def get_last_file(self,_type):
        ...
        db = MySQLdb.connect(self.hostname,'root','My$qlD',self.db_name)
        cursor = db.cursor()
	sql_2 = 'SELECT FILENUMBER from  PROCESSED where _type = "' + _type+'";'
	cursor.execute(sql_2)
	results = cursor.fetchall()
        #sql = 'SELECT FILE FROM '+self.table_name + ' '
        #sql = sql+'WHERE ITEM_TYPE="'+_type +'" limit 1;' #'" order by FILE desc limit 1;'
        
        #cursor.execute(sql)
        #results = cursor.fetchall()
        last_file_number = results[0][0] 
	max_number = len(results)
	file_name =  '/npd/ODS/ODS_INPUTS/ODS_POSOUTLETITEMS/part-m-' if _type is 'POIID' else  '/npd/ODS/ODS_INPUTS/ODS_POSITEMS/part-m-' 
	next_file = file_name + "{:05}".format(last_file_number) + '.gz'
	print next_file
        #next_file = last_file.split('-m-')[0] + '-m-'
        #filenumber = int(last_file.split('-m-')[1].replace('.gz',''))
        #next_file = next_file + '{:05}'.format(file_number[0][0] + 1) + '.gz'
        return(next_file,last_file_number)
    ...
    def start_new_changeid_dict(self,_data,## dictionary of lists
        ...
        for i,item_id in enumerate(itemid):
            ...
            job_id = self.set_changeid_fast(cursor,itemid[i],poiid[i],'',update_date[i],_quiet=_quiet)
    ...
    def update_multiple(self, *types , **params):
	db =  MySQLdb.connect(self.hostname,'root','My$qlD',self.db_name)

	sql = 'UPDATE ' + self.table_name + ' '
        sql = sql + ' SET ' 
	for ind,each in enumerate(params.iteritems()):
		sql = sql + each[0] + ' = ' + str(each[1]) 
		sql  = sql + ', ' if ind is not len(params)-1 else sql + ' '
	sql = sql + ' WHERE _TYPE ="' + types[0]+'";'
    ...
    def start_upc_search(self, _process_name):
        ...
        sql = 'SELECT '+self.JobID_name+' FROM '+self.table_name+' '
        #sql = sql+'WHERE UPC_SEARCHSTATUS_START="0000-00-00 00:00:00" and ITEM_TYPE="POIID" and ADDED_DATE="2017-08-13 00:00:00" limit 100;'
        sql = sql+'limit 100'
        
        cursor.execute(sql)
        results = cursor.fetchall()
        res_list = list(zip(*results)[0])

        sql = 'SELECT CHANGE_ID,UPC,SKU,POSOUTLET,OUTLETITEMNUMBER,OUTLETBRAND FROM POIID_DETAILS '
        sql = sql+'WHERE CHANGE_ID in ('+','.join(str(x) for x in res_list)+');'

        cursor.execute(sql)
        results = cursor.fetchall()
        res_list = list(zip(*results)[0])
        upc_list = list(zip(*results)[1])
        sku_list = list(zip(*results)[2])
        rtr_list = list(zip(*results)[3])
        mod_list = list(zip(*results)[4])
        bnd_list = list(zip(*results)[5])
        ...
        db.close()
        return({'ChID':res_list,'UPC':upc_list,'SKU':sku_list,'POSOUTLET':rtr_list,'MOD':mod_list,'BRAND':bnd_list})
